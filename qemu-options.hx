...
DEF("icount", HAS_ARG, QEMU_OPTION_icount, \
    "-icount [{shift=N|auto}|ns-per-insn=N][,align=on|off][,sleep=on|off][,rr=record|replay,rrfile=<filename>[,rrsnapshot=<snapshot>]]\n" \
    "                enable virtual instruction counter with 2^N or exactly N\n" \
    "                nanoseconds per instruction, enable aligning the host and virtual clocks\n" \
    "                or disable real time cpu sleeping, and optionally enable\n" \
    "                record-and-replay mode\n", QEMU_ARCH_ALL)
SRST
``-icount [{shift=N|auto}|ns-per-insn=N][,align=on|off][,sleep=on|off][,rr=record|replay,rrfile=filename[,rrsnapshot=snapshot]]``
    Enable virtual instruction counter. The virtual cpu will execute one
    instruction every 2^N ns of virtual time. Alternatively,
    ``ns-per-insn=N`` selects an exact integer number from 1 through 1024
    nanoseconds per instruction. ``shift`` and ``ns-per-insn`` are mutually
    exclusive. If ``auto`` is specified then the virtual cpu speed will be
