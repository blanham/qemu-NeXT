...
- [ ] Add failing tests for an exact 141-byte `AuthTreq` record, an exact 145-byte `AuthOK+2 tickets` reply, fixed `AuthErr` replies for malformed or unsupported requests, challenge/key agreement, `hostid==uid`, speaks-for rejection to `none`, privacy substitution, backpressure, and the historical boot's two sequential exchanges on one connection.
- [ ] Implement independent bounded auth connections. Generate a fresh seven-byte conversation key for each exchange and encrypt client/server tickets with the appropriate principal/server keys. Accept the next request only after the prior reply is atomically sent; after atomically sending `AuthErr`, close on malformed input. Also close on a request while a reply is pending, fatal transport failure, peer close, or the bounded exchange limit.
